function I = sparse( H_t_all, W_all_block, N_W, nLayers, receiver, N_0 )
...
N_rb = size(H_t_all,4); % number of resource blocks
I    = zeros(N_rb,N_W);
N_s = size(H_t_all,3);
N_TX        = size(H_t_all,2);
N_RX        = size(H_t_all,1);
...
for RB_i = 1:N_rb
    for s_i = 1:N_s % loop over OFDM symbols
        H_t = H_t_all(:,:,s_i,RB_i);
        ...
        noise       = reshape(N_0.*sum(abs(F_all).^2,2),nLayers,[]);
        SNR_all     = signal ./ (inter_layer + noise);
        I(RB_i,:)   = I(RB_i,:) + sum(log2(1+SNR_all),1); % rate of one resource block for different precoders and this specific rank (nLayers)
    end
end
end
