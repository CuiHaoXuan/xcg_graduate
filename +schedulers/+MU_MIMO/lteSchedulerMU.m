classdef lteSchedulerMU < schedulers.lteScheduler
    ...
    methods
        ...
        % Class constructor
        function obj = lteSchedulerMU(scheduler_params,attached_eNodeB)
            obj = obj@schedulers.lteScheduler(scheduler_params,attached_eNodeB)
            obj.cqi_reduction = scheduler_params.cqi_reduction;
        end
        ...
        function [N_assigned_RBs, CQIs_to_average_all, UE_scheduled, new_UE_RB_map] = ...
            ...
            if obj.skip_null_CQIs
                if nCodewords == 1
                    ...
                    non_valid_RBs = logical(UE_RBs.*...
                        (kron(ones(1,size(UE_RBs, 2)),sum(UE_RBs,2) & zero_CQIs)));  % RBs to filter out
                    valid_RBs     =  logical(UE_RBs.*...
                        (kron(ones(1,size(UE_RBs, 2)),sum(UE_RBs,2) & ~zero_CQIs))); % CQIs that will be averaged
                    RB_grid.user_allocation(non_valid_RBs) = 0;
                    CQIs_to_average_all = UE_CQI_feedback(logical(sum(valid_RBs, 2)));
                else
                    % For the case where more than 1 codewords are sent, all CWs must have a CQI >0
                    zero_CQIs     = sum(UE_CQI_feedback<1,2)>=1;
                    non_valid_RBs = sum(UE_RBs,2) & zero_CQIs; % RBs to filter out
                    valid_RBs     = sum(UE_RBs,2) & ~zero_CQIs; % CQIs that will be averaged
                    RB_grid.user_allocation(non_valid_RBs) = 0;
                    CQIs_to_average_all = UE_CQI_feedback(valid_RBs,:);
                end
                new_UE_RB_map  = valid_RBs;
                N_assigned_RBs = sum(sum(valid_RBs));
            else
                if nCodewords == 1
        ...
        function schedule_users_common(obj,attached_UEs,UE_feedback,current_TTI,tx_mode)
            ...
            if ~isempty(attached_UEs)
                ...
                mu_mimo_weights = kron(ones(1, the_RB_grid.max_ues),sum(scheduled_idxs, 2));
                the_RB_grid.power_allocation = scheduled_idxs.*power_per_rb./mu_mimo_weights;
            end
            ...
            for u_=1:length(attached_UEs)
                ...
                if UE_feedback.feedback_received(u_)
                    ...
                    if tx_mode==1||tx_mode==2
                        ...
                        nLayers    = 1;
                        nCodewords = 1;
                    elseif tx_mode==3||tx_mode==4 % OLSM, CLSM, and others
                    ...
                    if UE_scheduled
                        ...
                        attached_UEs(u_).eNodeB_signaling.assigned_power               = sum(sum(total_RB_power(RB_map)));
                        ...
                        if obj.runtime_precoding
                            ...
                            elseif  sum(tx_mode==[4 5 6])
                                layer_codebook = obj.Rel8_codebook{nLayers,obj.attached_eNodeB.total_nTX,tx_mode};
                                RB_list        = find(sum(RB_map,2));
                                for rb_idx=1:length(RB_list)
                                    rb_ = RB_list(rb_idx);
                                    prec = layer_codebook.W(:,:,UE_feedback.PMI(rb_,u_));
                                    if isreal(prec)
                                        the_RB_grid.precoder(find(RB_map(rb_, :)), rb_).W = complex(prec);
                                    else
                                        the_RB_grid.precoder(find(RB_map(rb_, :)), rb_).W = prec;
                                    end
                                end
                                %the_RB_grid.precoder(RB_map) = obj.get_precoder_map(nLayers, obj.attached_eNodeB.total_nTX, tx_mode, RB_map, UE_feedback.PMI(:, u_));
                        ...
                        if tx_mode == 5
                            attached_UEs(u_).eNodeB_signaling.assigned_RB_map              = zeros(size(the_RB_grid.user_allocation));
                        else
                    ...
                    num_assigned_RB = sum(sum(RB_map));
                    nCodewords      = 1;
                    nLayers         = 1;
                ...
                %the_RB_grid.precoder(~the_RB_grid.user_allocation) = default_precoder;
                % Calculate TB size
                if UE_scheduled
                    TB_CQI = attached_UEs(u_).eNodeB_signaling.TB_CQI;
                    if sum((mu_mimo_weights(RB_map))> 1)>0
                        TB_CQI = max(TB_CQI-obj.cqi_reduction, 1);
                        attached_UEs(u_).eNodeB_signaling.TB_CQI = TB_CQI;
                    end
                    ...
                    if mod(current_TTI-1,5) || strcmp(class(obj.attached_eNodeB.scheduler),'schedulers.ffrScheduler') % TB without sync symbols. Solution (for now) for FFR
                        ...
                        sync_RBs = sum(sum((the_RB_grid.user_allocation==attached_UEs(u_).id) .* sync_pos));
                        non_sync_RBs = sum(sum(the_RB_grid.user_allocation==attached_UEs(u_).id))-sync_RBs;
        ...
        function precoders = get_precoder_map(obj, nLayers, total_nTX, tx_mode, RB_map, PMI)
        
            layer_codebook = obj.Rel8_codebook{nLayers,total_nTX,tx_mode};
            RB_list        = find(sum(RB_map,2));
            %precoders = zeros(size(RB_list));
            
            for rb_idx=1:length(RB_list)
                rb_ = RB_list(rb_idx);
                for u_ = 1:size(PMI, 2)
                    precoders(rb_idx, u_).W = layer_codebook.W(:,:,PMI(rb_, u_));
                    
                    if isreal(precoders(rb_idx, u_).W)
                        precoders(rb_idx, u_).W = complex(precoders(rb_idx, u_).W);
                    end
                end
            end
        end
        function orthogonal_UEs = get_orthogonal_UEs(obj, check_matrix, attached_UEs, input_UE)
            % This function returns the group of UEs that are orthogonal to the input UE(s)
            % and are all mutually orthogonal
            
            if isempty(check_matrix)
                orthogonal_UEs = [];
                return;
            end
            % Step 1: find all UEs that have orthogonal precoders to the
            % given set of UEs
            filter_idx = logical(sum(bsxfun(@eq, [attached_UEs.id], reshape(input_UE, [], 1)), 1));
            reduced_mat = check_matrix(filter_idx, :);
            candidate_idx =~logical(sum(reduced_mat==0, 1));
            
            % Step 2: Look for the largest group of mutually orthogonal
            % precoders
            if sum(candidate_idx) > 1
                 candidate_UEs = attached_UEs(candidate_idx);
                orthogonal_UEs = [candidate_UEs.id];
            elseif sum(candidate_idx) == 1
                orthogonal_UEs = attached_UEs(candidate_idx).id;
            else 
                orthogonal_UEs = [];
            end
