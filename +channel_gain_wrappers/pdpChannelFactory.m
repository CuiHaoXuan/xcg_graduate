classdef pdpChannelFactory < channel_gain_wrappers.channelFactory
    ...
    methods (Static)
        function pdp_params = get_default_config(carrier_freq,nTX,nRX,speed,correlated,channel)
            pdp_params.BS_config.nTx     = nTX;
            pdp_params.UE_config.nRX     = nRX;
            pdp_params.speed             = speed;  % m/s
            pdp_params.correlated_fading = correlated;
    ...
    methods
        function obj = pdpChannelFactory(bandwidth,pdp_params)
            ...
            obj.fading_config_params.Ns                    = pdp_params.Ns; % number of OFDM symbols per subframe
            ...
            if isfield(pdp_params,'BF_samples')
                obj.fading_config_params.symbol_times = pdp_params.BF_samples; % OFDM symbol indices for short block fading case
            else
                obj.fading_config_params.symbol_times = 0;
            end
        end
        ...
        function ChanMod = ChanMod_params_generator(obj,channel_type,nTX,nRX)
            ...
            if strcmp(ChanMod.type,'PedA') || strcmp(ChanMod.type,'PedB') || strcmp(ChanMod.type,'VehA') || strcmp(ChanMod.type,'VehB') ||strcmp(ChanMod.type,'TU') || strcmp(ChanMod.type,'RA') || strcmp(ChanMod.type,'HT') || strcmp(ChanMod.type,'extPedB') || strcmp(ChanMod.type, 'Rayleigh')
        ...
        function pregen_H_trace = generate_FF_trace(obj,N_subframes)
            ...
            pregen_H_trace.H_RB_samples       = zeros(nRX,nTX,length(obj.fading_config_params.symbol_times),N_subframes,obj.fading_config_params.Ntot/obj.FFT_sampling_interval);
            ...
            for loop_idx = 1:number_of_loops
                ...
                fading_gen_output = obj.fading_generation(loop_offset); % out of some reason this is fixed to 2000... what if I don't need so many subframes?               
                pregen_H_trace.H_RB_samples(:,:,:,samples,:) = fading_gen_output(:,:,:,1:length(samples),:);
            end
            fprintf('\n');
        end
...
%         end
        ...
        function H_fft_to_return = fading_generation(obj,loop_offset)
            ...
            FFT_sampling_interval  = fading_config_params.FFT_sampling_interval; % Specifiy out of the useful data subcarrier's FFT,
            % which ones to take (all of them take up too much memory)
            Ns                     = fading_config_params.Ns; % number of OFDM symbols per subframe
            symbol_times           = (fading_config_params.symbol_times-1)*1/Ns;
            tot_number_of_realizations = number_of_realizations*length(symbol_times);

            %% Preallocation of the Time matrix
            time_i = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
            time_i_help = tap_delays(tap_position) / fs;
  
            time_i_help = kron(time_i_help, ones(tot_number_of_realizations,1)) + (kron(ones(1,number_of_realizations),symbol_times) + kron((0:(number_of_realizations-1))+loop_offset,ones(1,length(symbol_times)))).'*ones(1,length(time_i_help))*Tsubframe;
            time_i(1,1,:,:,1) = time_i_help;
            time_i = repmat(time_i(1,1,:,:,1),[nRX, nTX, 1, 1, M]);            
            ...
            if correlated_fading
                ...
                c = 299792458;
                w_d   = 2*pi*v*f/c;      % Maximum radian Doppler frequency
                X_c   = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                X_s   = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                psi_n = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                theta = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                phi   = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                ...
                psi_n = repmat(psi_n(:,:,1, :, :), [1,1,tot_number_of_realizations, 1, 1]);
                phi = repmat(phi(:,:,1, :, :), [1,1,tot_number_of_realizations, 1, 1]);
                theta = repmat(theta(:,:,1, :, :), [1,1,tot_number_of_realizations, 1, 1]);
                
                PI_mat = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps,M);
                PI_mat(1,1,1,1,:) = (1:M)*2*pi;
                PI_mat = repmat(PI_mat(1,1,1,1,:), [nRX, nTX, tot_number_of_realizations, number_of_taps, 1]);
                alpha_n = (PI_mat - pi + theta) / (4*M);
                ...
                X = (randn(nRX,nTX,tot_number_of_realizations,number_of_taps)+1i*randn(nRX,nTX,tot_number_of_realizations,number_of_taps))/sqrt(2);
            end
            
            weight_matrix = zeros(nRX, nTX,tot_number_of_realizations,number_of_taps);
            weight_matrix(1,1,:,:) = kron(tap_powers_used(unique(tap_delays+1)), ones(tot_number_of_realizations,1));
            weight_matrix = repmat(weight_matrix(1,1,:,:),[nRX, nTX, 1, 1]);
            H_temp = zeros(nRX, nTX, tot_number_of_realizations, tap_delays(end)+1);
            H_temp(:,:,:, tap_delays(tap_position)+1) = X .* weight_matrix;
            H = zeros(nRX, nTX,length(symbol_times),number_of_realizations,tap_delays(end)+1);
            H = reshape(H_temp,nRX,nTX,length(symbol_times),number_of_realizations,tap_delays(end)+1);

            %% Trace in freq domain
            H_fft_to_return = obj.get_RB_trace(H);
%             plot([abs(squeeze(H_fft_to_return(1,1,:,1,1)));abs(squeeze(H_fft_to_return(1,1,:,2,1)));abs(squeeze(H_fft_to_return(1,1,:,3,1)))])
%                 error('j')
        end
    end
end
