classdef winnerChannelFactory < channel_gain_wrappers.channelFactory
    ...
    properties
        ...
        samples_per_subframe % specifies the number of channel samples taken per subframe
    end
    ...
    methods (Static)
        function winner_params = get_default_config(carrier_freq,nTX,nRX,speed)
            ...
            winner_params.Scenario               = 11;         % 1=A1, 2=A2, 3=B1, 4=B2, 5=B3, 6=B4, 7=B5a, 8=B5c, 9=B5f, 10=C1, 11=C2, 12=C3, 13=C4, 14=D1 and 15=D2a
    ...
    methods
        function obj = winnerChannelFactory(bandwidth,winner_params,N_per_subframe,varargin)
            ...
            obj.samples_per_subframe  = N_per_subframe;
        end
            ...
            switch LTE_params.UE_config.nRX
                ...
                case 2
                    ...
                    switch LTE_params.BS_config.nTx
                        ...
                        otherwise
                            pattern(1,:,1,:) = dipole(Az,RX_pol(1));
                            pattern(2,:,1,:) = dipole(Az,RX_pol(2));
                    end
                ...
                case 4
                    ...
                    pattern(1,:,1,:) = dipole(Az,RX_pol(1));
                    pattern(2,:,1,:) = dipole(Az,RX_pol(2));
                    pattern(3,:,1,:) = dipole(Az,RX_pol(3));
                    pattern(4,:,1,:) = dipole(Az,RX_pol(4));
                    UserArrays       = AntennaArray('Pos',Position,'Rot',Rotation,'FP-ECS',pattern,'Azimuth',Az);
                otherwise
        ...
        function [channel_out, delays, out, normalization_factor] = generate_channel_trace(obj,N_subframes,varargin)
            ...
            switch init_params.filtering
                case 'BlockFading'
                    Channel_Sampling_Time = init_params.Tsubframe/obj.samples_per_subframe;
                case 'FastFading'
            ...
            wimpar.NumTimeSamples          = N_subframes*obj.samples_per_subframe;  % number of time samples [ {100} ]
        ...
        function pregen_H_trace = generate_FF_trace(obj,N_subframes)
            ...
            pregen_H_trace.H_RB_samples       = zeros(nRX,nTX,obj.samples_per_subframe,N_subframes,obj.Ntot/obj.FFT_sampling_interval);
            ...
            for i_=1:number_of_loops
                ...
                size_chan = size(channel);
                channel = reshape(channel,[size_chan(1),size_chan(2),obj.samples_per_subframe,size_chan(3)/obj.samples_per_subframe,size_chan(4:end)]);
                samples = begin_pos(i_):end_pos(i_);
                pregen_H_trace.H_RB_samples(:,:,:,samples,:) = obj.get_RB_trace(channel);
%                 plot([abs(squeeze(pregen_H_trace.H_RB_samples(1,1,:,1,1)));abs(squeeze(pregen_H_trace.H_RB_samples(1,1,:,2,1)));abs(squeeze(pregen_H_trace.H_RB_samples(1,1,:,3,1)))],'r')
%                 error('j')
            end
        end
    end
