classdef TPvsSNR < utils.simulatorConfig
    methods (Static)
        function LTE_config = apply_parameters(LTE_config)
            ...
            %% General options
            LTE_config.frequency       = 2.1e9;         % Frequency in Hz
            LTE_config.bandwidth       = 1.4e6;           % Frequency in Hz
            
            LTE_config.nTX           = 2;
            LTE_config.nRX           = 2;
            LTE_config.tx_mode       = 4; % CLSM
            LTE_config.BF_short      = true;
            LTE_config.ICI_emulation = true;
            ...
            %% Simulation time
            LTE_config.simulation_time_tti = 1100; % Simulation time in TTIs
            ...
            switch LTE_config.network_geometry
                case 'regular_hexagonal_grid' % regular_hexagonal_grid params
                    LTE_config.inter_eNodeB_distance = 1000; % In meters. When the network is generated, this determines the distance between the eNodeBs.
            ...
            switch LTE_config.network_source
                case 'generated'
                    ...
                    LTE_config.minimum_coupling_loss = 0; % Minimum Coupling Loss: the parameter describing the minimum
                    ...
                    % eNodeB settings
                    LTE_config.eNodeB_tx_power = 1; % eNodeB's transmit power, in Watts.
            ...
            LTE_config.channel_model.type = 'winner+'; % 'TU' % 'winner+' 'PedB' 'extPedB' 'TU' 'Gamma' --> the PDP to use 
            LTE_config.channel_model.trace_length = 3; % Length of the trace in seconds. Be wary of the size you choose, as it will be loaded in memory.
            ...
            LTE_config.UE.thermal_noise_density = -139.1;  % Thermal noise density in dBm/Hz (-174 dBm/Hz is the typical value)
            
            LTE_config.UE_distribution = 'constant UEs per cell';
            LTE_config.UE_per_eNodeB   = 5;    % number of users per eNodeB sector (calculates it for the center sector and applies this user density to the other sectors)
